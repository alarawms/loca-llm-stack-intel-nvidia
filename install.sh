...
if [[ $SKIP_BUILD -eq 0 ]]; then
    ...
    podman pull "$LLM_ARC_OPENCLAW_IMAGE"
    log_ok "OpenClaw image pulled"
else
...
mkdir -p "$LLM_ARC_OLLAMA_MODELS" \
         ...
         "${LLM_ARC_MODELS}/openclaw/config" \
         "${LLM_ARC_MODELS}/openclaw/plugins"

# Generate OpenClaw gateway token on first install
openclaw_env="${LLM_ARC_MODELS}/openclaw/openclaw.env"
...
EOF
    ...
    log_ok "OpenClaw gateway token generated"
fi
...
if [[ $SKIP_START -eq 0 ]]; then
    ...
    if curl -sf "http://127.0.0.1:${LLM_ARC_OPENCLAW_PORT}/" >/dev/null 2>&1; then
        log_ok "OpenClaw agent responding"
        (( ++smoke_ok ))
    else
        log_warn "OpenClaw not responding (may still be loading)"
    fi
...
echo "    http://localhost:${LLM_ARC_WEBUI_PORT}   Open WebUI (chat)"
echo ""
echo "  ${BOLD}Optional: Dify AI Platform + Qdrant (agents, RAG, workflows)${RESET}"
echo "    ./scripts/dify-stack.sh install"
echo "    ./scripts/dify-stack.sh start"
echo "    http://localhost:${LLM_ARC_DIFY_PORT}     Dify UI"
echo "    http://localhost:${LLM_ARC_QDRANT_PORT}   Qdrant REST API"
echo "    In Dify → Settings → Model Providers → Ollama"
echo "    Ollama endpoint: http://host.containers.internal:${LLM_ARC_OLLAMA_PORT}"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
